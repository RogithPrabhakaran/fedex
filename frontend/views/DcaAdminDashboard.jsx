...
const DcaAdminDashboard = () => {
  ...
  const safeAgencies = useMemo(
    () =>
      Array.isArray(agencies) && agencies.length > 0 ? agencies : mockAgencies,
    [agencies]
  );
  ...
  const safeSummary = useMemo(
    () =>
      Array.isArray(casesSummary) && casesSummary.length > 0
        ? casesSummary
        : mockCasesSummary,
    [casesSummary]
  );
  ...
  const top3Agencies = useMemo(() => {
    return [...safeAgencies]
      .sort((x, y) => (y?.recovery_rate || 0) - (x?.recovery_rate || 0))
      .slice(0, 3);
  }, [safeAgencies]);

  const totals = useMemo(() => {
    const totalPaymentDue = safeCustomers.reduce(
      (s, cu) => s + (Number(cu?.totalDebt) || 0),
      0
    );
    const totalRecovered = safeAgencies.reduce(
      (s, a) => s + (Number(a?.recovered_amount) || 0),
      0
    );
    ...
    const topCustomers = [...safeCustomers]
      ...
      .slice(0, 8);
    const avgRecoveryRate = Math.round(
      (safeAgencies.reduce((s, a) => s + (Number(a?.recovery_rate) || 0), 0) /
        safeAgencies.length) *
        100
    );
  ...
  const monthlySeries = useMemo(() => {
    ...
    return [
      {
        name: 'Recovered',
        data: sorted.map((r) => Number(r?.recovered_amount) || 0),
      },
    ];
  }, [safeSummary]);
    ...
    <div className='p-4 md:p-10 max-w-[1400px] mx-auto'>
      <div className='flex items-center justify-between mb-6'>
        <div>
          <h1 className='text-3xl font-black text-slate-900 dark:text-white'>
            <Translate text='DCA Admin Dashboard' />
          </h1>
          <p className='text-slate-400'>
            <Translate text='Overview of agency performance, recovery progress, and top accounts.' />
          </p>
        </div>
      </div>
      ...
      <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-6'>
        <div className='p-6 bg-gradient-to-br from-blue-500/10 to-blue-600/5 dark:from-blue-500/20 dark:to-blue-600/10 border border-blue-200 dark:border-blue-500/30 rounded-2xl'>
          <div className='text-slate-400 text-xs font-black uppercase'>
            Total Value At Risk
          </div>
          <div className='text-2xl font-black text-slate-900 dark:text-white mt-2'>
            {numberFormat(totals.totalPaymentDue)}
          </div>
          <div className='text-sm text-slate-500 mt-1'>
            {safeCustomers.length} Debtors
          </div>
        </div>
        ...
        <div className='p-6 bg-gradient-to-br from-green-500/10 to-green-600/5 dark:from-green-500/20 dark:to-green-600/10 border border-green-200 dark:border-green-500/30 rounded-2xl'>
          ...
          <div className='text-2xl font-black text-slate-900 dark:text-white mt-2'>
            {numberFormat(totals.totalRecovered)}
          </div>
          <div className='text-sm text-green-600 dark:text-green-400 mt-1'>
            {totals.avgRecoveryRate}% Avg Recovery
          </div>
        </div>
        ...
        <div className='p-6 bg-gradient-to-br from-purple-500/10 to-purple-600/5 dark:from-purple-500/20 dark:to-purple-600/10 border border-purple-200 dark:border-purple-500/30 rounded-2xl'>
          ...
          <div className='text-2xl font-black text-slate-900 dark:text-white mt-2'>
            {safeAgencies.reduce((s, a) => s + (a?.active_cases || 0), 0)}
          </div>
          <div className='text-sm text-slate-500 mt-1'>
            Across {safeAgencies.length} Agencies
          </div>
        </div>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-3 gap-6'>
        ...
        <div className='col-span-1 space-y-6'>
          <div className='p-6 bg-white dark:bg-surface-dark border border-slate-200 dark:border-surface-border rounded-2xl'>
            <h3 className='text-lg font-black text-slate-900 dark:text-white mb-4'>
              <Translate text='Top Performing Agencies' />
            </h3>
            <div className='space-y-3'>
              {top3Agencies.map((a, idx) => (
                <div
                  key={a?.dca_id || idx}
                  className='flex items-start gap-3 p-4 rounded-xl bg-gradient-to-r from-slate-50 to-white dark:from-slate-800/50 dark:to-slate-900/30 border border-slate-200 dark:border-surface-border hover:border-fedex-orange/30 transition-colors'
                >
                  <div className='flex-shrink-0 w-8 h-8 rounded-full bg-fedex-orange/20 flex items-center justify-center'>
                    <span className='text-sm font-black text-fedex-orange'>
                      {idx + 1}
                    </span>
                  </div>
                  <div className='flex-1 min-w-0'>
                    <div className='font-bold text-slate-900 dark:text-white text-sm'>
                      {a?.name || 'Unknown Agency'}
                    </div>
                    ...
                    <div className='mt-2 grid grid-cols-2 gap-2 text-xs'>
                      ...
                      <div className='bg-white/50 dark:bg-white/5 px-2 py-1 rounded text-right'>
                        <span className='text-slate-500'>
                          {numberFormat(a?.recovered_amount || 0)}
                        </span>
                      </div>
                    </div>
                  </div>
          ...
          <div className='p-6 bg-white dark:bg-surface-dark border border-slate-200 dark:border-surface-border rounded-2xl overflow-auto'>
            <h3 className='text-lg font-black text-slate-900 dark:text-white mb-4'>
              <Translate text='High-Value Accounts' />
            </h3>
            <table className='w-full text-sm'>
              ...
              <tbody>
                {totals.topCustomers.map((cu) => (
                  <tr
                    key={cu?.id}
                    className='border-b border-slate-100 dark:border-surface-border/50 hover:bg-slate-50 dark:hover:bg-white/5'
                  >
                    <td className='py-3 text-slate-900 dark:text-white font-bold text-sm'>
                      {cu?.name || 'Unknown'}
                    </td>
                    <td className='py-3 text-right text-slate-600 dark:text-slate-300 font-semibold'>
                      {numberFormat(cu?.totalDebt)}
                    </td>
        ...
        <div className='col-span-1 space-y-6'>
          <div className='p-6 bg-white dark:bg-surface-dark border border-slate-200 dark:border-surface-border rounded-2xl'>
            <h3 className='text-lg font-black text-slate-900 dark:text-white mb-4'>
              <Translate text='Agency Performance' />
            </h3>
            <div className='space-y-3'>
              {safeAgencies.map((a, idx) => (
                <div
                  key={a?.dca_id || idx}
                  className='flex items-start justify-between gap-3 p-3 rounded-lg hover:bg-slate-50 dark:hover:bg-white/5 transition-colors'
                >
                  <div className='flex-1 min-w-0'>
                    ...
                    <div className='flex gap-2 mt-1'>
                      <span className='text-xs bg-blue-500/20 text-blue-600 px-2 py-0.5 rounded'>
                        Score: {a?.performance_score?.toFixed(1)}
                      </span>
            ...
            <div className='space-y-3 text-sm'>
              ...
              <div className='flex justify-between items-center pb-3 border-b border-slate-200 dark:border-surface-border'>
                ...
                <span className='font-black text-slate-900 dark:text-white'>
                  {Math.round(
                    safeAgencies.reduce(
                      (s, a) => s + (a?.avg_days_to_recovery || 0),
                      0
                    ) / safeAgencies.length
                  )}{' '}
                  days
