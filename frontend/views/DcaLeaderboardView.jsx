...
const DcaLeaderboardView = () => {
  ...
  const filtered = useMemo(() => {
    ...
    if (filterText) {
      ...
      list = list.filter(
        (a) =>
          (a.agency_name || a.name || '').toLowerCase().includes(t) ||
          (a.regions || '').toLowerCase().includes(t)
      );
    }
    if (statusFilter !== 'ALL') {
  ...
  return (
    <div className='p-6 max-w-6xl mx-auto'>
      <div className='flex items-center justify-between mb-6'>
        <div>
          <h1 className='text-3xl font-black text-slate-900 dark:text-white'>
            <Translate text='DCA Leaderboard' />
          </h1>
          <p className='text-slate-400'>
            <Translate text='Ranking of DCA partners and quick stats for actions this month.' />
          </p>
        </div>
      </div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-3 gap-4 mb-6'>
        <StatCard
          title='Top Performing DCA (This Month)'
          value={
            topPerforming[0]
              ? `${topPerforming[0].name || topPerforming[0].agency_name}`
              : '—'
          }
          subtitle={
            topPerforming[0]
              ? `${numberFormat(
                  topPerforming[0].recovered_amount ||
                    topPerforming[0].recovered_amount
                )} • ${(topPerforming[0].recovery_rate || 0).toFixed(1)}%`
              : ''
          }
        />
        <StatCard
          title='Total Recovered (All DCAs)'
          value={numberFormat(
            agencies.reduce((s, a) => s + (Number(a.recovered_amount) || 0), 0)
          )}
          subtitle={`${agencies.length} agencies`}
        />
        <StatCard
          title='Average Recovery Days'
          value={
            agencies.length
              ? `${Math.round(
                  agencies.reduce(
                    (s, a) => s + (Number(a.avg_days_to_recovery) || 0),
                    0
                  ) / agencies.length
                )} days`
              : '—'
      ...
      <div className='bg-white dark:bg-surface-dark p-4 rounded-2xl border border-slate-200 dark:border-surface-border mb-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4'>
        <div className='flex items-center gap-3'>
          <input
            ...
            className='p-2 rounded-md bg-[#0f1316] text-white px-4'
          />
          <select
            value={statusFilter}
            onChange={(e) => setStatusFilter(e.target.value)}
            className='p-2 rounded-md bg-[#0f1316] text-white'
          >
        ...
        <table className='w-full text-sm'>
          <thead className='text-slate-400 text-xs text-left'>
            <tr>
              ...
              <th className='py-2'>
                <Translate text='Recovery Rate' />
              </th>
              <th className='py-2'>
                <Translate text='Recovered' />
              </th>
              <th className='py-2'>
                <Translate text='Cases' />
              </th>
              <th className='py-2'>
                <Translate text='Avg Days' />
              </th>
              <th className='py-2'>
                <Translate text='Status' />
              </th>
              <th className='py-2'>
                <Translate text='Commission %' />
              </th>
              <th className='py-2'>
          ...
          <tbody>
            ...
            {!loading && filtered.length === 0 && (
              <tr>
                <td colSpan={10} className='py-6 text-center text-slate-500'>
                  <Translate text='No agencies found' />
                </td>
              </tr>
            )}
            ...
            {!loading &&
              filtered.map((a, idx) => (
                <tr
                  key={a.dca_id || a.id || idx}
                  className='border-t border-slate-200 dark:border-surface-border'
                >
                  <td className='py-3'>{idx + 1}</td>
                  <td className='py-3 text-slate-900 dark:text-white font-bold'>
                    {a.name || a.agency_name}
                  </td>
                  <td className='py-3 text-slate-400'>
                    {a.regions || 'Global'}
                  </td>
                  <td className='py-3 text-slate-900 dark:text-white'>
                    {(a.recovery_rate || 0).toFixed(1)}%
                  </td>
                  <td className='py-3 text-slate-300'>
                    {numberFormat(a.recovered_amount || a.recovered_amount)}
                  </td>
                  <td className='py-3 text-slate-300'>
                    {a.total_cases_handled || a.cases_handled || '—'}
                  </td>
                  <td className='py-3 text-slate-300'>
                    {a.avg_days_to_recovery || a.avg_days_to_recovery || '—'}
                  </td>
                  <td className='py-3'>
                    <span
                  ...
                  <td className='py-3 text-slate-300'>
                    {a.commission_rate != null ? `${a.commission_rate}%` : '—'}
                  </td>
                  <td className='py-3'>
                    <button
                      onClick={() =>
                        alert(
                          'Open agency details for ' + (a.name || a.agency_name)
                        )
                      }
                      className='px-3 py-1 bg-[#111418] rounded-md text-white border border-slate-200 dark:border-surface-border'
                    >
                      View
                    </button>
                  </td>
